...
const URL = "https://teachablemachine.withgoogle.com/models/b7Qrarn2e/";
...
async function init() {
    ...
    // 2. ตั้งค่า Webcam
    const flip = true; // ตั้งค่าให้ภาพกลับด้านแนวนอน (เหมาะสำหรับกล้องหน้า)
    try {
        webcam = new tmImage.Webcam(200, 200, flip); // กว้าง, สูง, กลับด้าน
        await webcam.setup(); // ขออนุญาตเข้าถึงกล้อง
        ...
        if (i === 0) { // Class ที่มีค่าความน่าจะเป็นสูงสุดอยู่เสมอ
            ...
            if (topProbability > 0.7) { // ปรับค่า 0.7 ได้ตามความเหมาะสมสำหรับไฮไลท์
                ...
                if (predictionHistory.length > 0) {
                    ...
                    isConsistent = false; // ถ้าไม่มีประวัติก็ไม่ถือว่า Consistent
                }


                // Logic สำหรับการแสดงผลพิเศษ (D1, D2)
                if (isConsistent) {
                    resultDisplayElement.className = 'important-message';
                    if (topClassName === "D1") {
                        resultDisplayElement.innerHTML = "<h3>🚨 เป็นโรคจุดราขาว 🚨</h3>";
                    } else if (topClassName === "D2") {
                    ...
                    if (topClassName === "Dog") {
                        resultDisplayElement.innerHTML = "<h4>🐶 นี่คือสุนัข!</h4><p>สุนัขเป็นสัตว์เลี้ยงที่ซื่อสัตย์และฉลาด!</p>";
                    } else if (topClassName === "Cat") {
                        resultDisplayElement.innerHTML = "<h4>🐱 นี่คือแมว!</h4><p>แมวเป็นสัตว์เลี้ยงที่น่ารักและรักอิสระ!</p>";
                    } else if (topClassName === "Chair") {
                        resultDisplayElement.innerHTML = "<h4>🪑 นี่คือเก้าอี้!</h4><p>เก้าอี้ใช้สำหรับนั่งพักผ่อนหรือทำงาน</p>";
                    } else if (topClassName === "Table") {
                        resultDisplayElement.innerHTML = "<h4>โต๊ะ!</h4><p>โต๊ะเป็นเฟอร์นิเจอร์ที่ใช้สำหรับวางสิ่งของหรือทำงาน</p>";
                    }
                    // *** คุณสามารถเพิ่ม `else if` ได้เรื่อยๆ สำหรับ Class อื่นๆ ในโมเดลของคุณ ***
                    else {
...
async function stopCamera() {
    isPredicting = false; // หยุด loop การทำนาย
    if (webcam) {
    ...
    resultDisplayElement.innerHTML = ''; // ล้างข้อมูลผลลัพธ์เพิ่มเติม
